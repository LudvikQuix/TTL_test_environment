...
# TTL build (sc-73191 branch, header-signal fix) for the cold-restore migration
# test: upgrade onto the v7.6 store the genuine 3.23.6 seeder filled. The first
# ttl= write flips the partition + backfills the pre-existing un-stamped records
# (legacy_records_ttl) and produces the re-stamped values to the changelog.
...
app = Application(
    consumer_group="dedup-filter-stable-v7.6",
    state_dir=STATE_DIR,
...
def dedup_filter(value, key, timestamp, headers, state):
    ...
    # First ttl= write on the populated v7.6 store flips the partition and
